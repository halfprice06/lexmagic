...
# Load the data
file_path = 'ccrp_article_urls.csv'  # Update this path
...
c.execute('''
    CREATE TABLE IF NOT EXISTS ccrp_articles (
        article_number TEXT,
        url TEXT,
        title TEXT,
        content TEXT
...
for index, row in data.iterrows():
    ...
    if article_content:
        c.execute('''
            INSERT INTO ccrp_articles (article_number,url,title,content)
            VALUES (?, ?, ?, ?)
        ''', (row['ccrp_article_number'], row['URL'], row['article_title'], article_content))
