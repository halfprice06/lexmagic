...
# Load the data
file_path = 'ccp_articles.csv'  # Update this path
...
c.execute('''
    CREATE TABLE IF NOT EXISTS ccp_articles (
        article_number TEXT,
        url TEXT,
        title TEXT,
        content TEXT
...
for index, row in data.iterrows():
    ...
    if article_content:
        c.execute('''
            INSERT INTO ccp_articles (article_number,url,title,content)
            VALUES (?, ?, ?, ?)
        ''', (row['CCP'], row['URL'], row['article_title'], article_content))
